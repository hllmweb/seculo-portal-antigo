...
class Boleto extends CI_Controller {
    ...
    function barras() {
        ...
        $this->codigobarras->generate($barras, 0, 'boleto.gif', 450, 60, false);
    ...
    function imprimir() {
        ...
        if ($data['boleto'][0]['DT_VENCIMENTO'] != NULL) {
            ...
            //$dados->numero_documento = $data['boleto'][0]['CD_ALUNO'] . $item[1] . $item[0]; // Num do pedido ou nosso numero
            $dados->numero_documento = $data['boleto'][0]['CD_BOLETO'];
            ...
            $dados->cidade_uf = $data['boleto'][0]['CIDADE'] . '/' . $data['boleto'][0]['UF'];
            $dados->cedente = $data['boleto'][0]['NM_MANTENEDORA'];
            $dados->res_endereco = $data['boleto'][0]['RES_ENDERECO'];
            ...
            include_once APPPATH . '/third_party/mpdf/mpdf.php';
            $mpdf = new mPDF('UTF8', // mode - default ''
...
function imprimir_brad_vencido(){
        $id = $this->input->get_post('id');
        $arr = explode("-",$id);
       ...
       if $data['boleto'][0]['DT_VENCIMENTO'] < date("d/m/Y") 
          {
            echo "vencido";
          }
          else

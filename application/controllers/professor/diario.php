...
class Diario extends CI_Controller {
    ...
    function index() {
        ...
        //echo '<pre>' . var_export($manha, true) . '</pre><br/>';
    ...
    function modal_frequencia() {
        ...
        $geminadas = $this->aula->listaAulasGeminada($codigo);
        if(!$geminadas){
            $geminadas = $aula;
        }

        //$aulasGeminada = $this->aula->consultar($codigo);

        //verificar o tipo de turma da frequência
        $turmas = array();
        foreach ($geminadas as $key => $value) {
            $turmas[$key] = $this->turma->consultar($value['CD_TURMA'], $value['PERIODO']);
        }
        ...
        foreach ($turmas as $key => $turma) {
            if ($turma['TIPO'] == "+") {
                $alunos[] = $this->presenca->listaFrequenciaMista($geminadas[$key]['CD_CL_AULA']);
            } else {
                $alunos[] = $this->presenca->listaFrequenciaNormal($geminadas[$key]['CD_CL_AULA']);
            }
        }

        //echo '<pre>' . var_export($geminadas, true) . '</pre><br/>';
        //echo '<pre>' . var_export($turmas, true) . '</pre><br/>';
        //echo '<pre>' . var_export($alunos, true) . '</pre><br/>';
        // exit();
        $data = array(
    ...
    function salvar_frequencia() {
        ...
        // $qtdCodigos = $this->input->post("qtdturmas");
        // $codigos= array();
        // for ($i=0; $i <(int)$qtdCodigos ; $i++) {
        //     $codigos[] = $this->input->post("turma".(string)$i);
        //  }
        //
...
//        echo '<pre>' . var_export($alunos, true) . '</pre><br/>';
        ...
        foreach ($alunos as $key => $value) {
            foreach ($value as $id => $aluno) {
                $aux = array(
                    'CD_ALUNO' => $aluno['CD_ALUNO'],
                    'FLG_PRESENTE' => $this->input->post($aluno['CD_ALUNO']."_".$geminadas[$key]['CD_CL_AULA']),
                    // 'CD_CL_AULA' => $geminadas[$key]['CD_CL_AULA']
                );
                $frequencia[] = $aux;
            }
            // $frequencia[] = $aux;
            $freqturma[(string)$geminadas[$key]['CD_CL_AULA']] = $frequencia;
            $frequencia = array();
        }


//          echo '<pre>' . var_export($freqturma, true) . '</pre><br/>';
// exit();



        $status = true;
        foreach ($freqturma as $codigoTurma => $frequencia) {

            if ($this->presenca->hasChamadaRealizada($codigoTurma)) {
                 $status = $this->presenca->editar($codigoTurma, $frequencia);
            } else {
                 $status = $this->presenca->adicionar($codigoTurma, $frequencia);
            }
        }

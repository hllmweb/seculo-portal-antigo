...
class Boletos {
    ...
    function txtBanco() {
        ...
        switch ($this->codigo_banco) {
            ...
            case 341:
                ...
                break;
            case 237:
                $txt = 'ATÉ O VENCIMENTO, PREFERENCIALMENTE NO BANCO BRADESCO, APÓS, SOMENTE NO BANCO BRADESCO S/A';
                break;
        }
        return $txt;
    }
    ...
    function nossoNumero() {
        ...
        switch ($this->codigo_banco) {
            ...
            case 237:
                ...
                $digito = $this->formata_numero(substr($this->nosso_numero, 17, 1), 1, 0);
                $nossonumero = $this->carteira . '/' . $nosso . '-' . $digito;*/
                /*$nnum = formata_numero($this->carteira,2,0).formata_numero($this->nosso_numero,11,0);
                $dv_nosso_numero = digitoVerificador_nossonumero($nnum);
                $nossonumero = substr($nnum,0,2).'/'.substr($nnum,2).'-'.$dv_nosso_numero;*/
                /*$nosso = $this->formata_numero($this->carteira,2,0).$this->formata_numero($this->nosso_numero,11,0);
                $digito = $this->digitoVerificador_nossonumero($nosso);
                $nossonumero = substr($nosso,0,2).'/'.substr($nosso,2).'-'.$digito;*/
                $carteira = $this->formata_numero($this->carteira,2,0);
                $nosso = $this->formata_numero(substr($this->nosso_numero,0,11),2,0);
                $digito = $this->formata_numero(substr($this->nosso_numero, 11, 1), 1, 0);

                $nossonumero = $carteira."/".$nosso."-".$digito;
                break;
        }
        return $nossonumero;
    }
            ...
            case 237:
                ...
                /*$codigo_barras = $this->codigo_banco .
                    ...
                    /*$codigo_barras = $this->codigo_banco .
                        ...
                        $codigo_barras =    $this->codigo_banco.
                                            $this->moeda.
                                            $this->digitoVerificador_barra($codigo).
                                            $this->fator_vencimento().
                                            $this->valor_boleto.
                                            $this->agencia.
                                            $this->conta.
                        ...
                        $this->formata_numero($this->valor_boleto, 10, 0, "valor"). 
                        $this->carteira .
                        substr($this->nosso_numero, 0, 17) .
                        $this->modulo_10($this->agencia.$this->conta.$this->carteira.
                        substr($this->nosso_numero, 0, 17)).
                        $this->agencia.
                        $this->conta.
                        $this->modulo_10($this->agencia . $this->conta).'000';
                        ...
                        $nummoeda           = $this->moeda;   
                        $fator_vencimento   = $this->fator_vencimento($this->vencimento);
                        $valor              = $this->formata_numero($this->valor_boleto,10,0,"valor"); 
                        $agencia            = $this->formata_numero($this->agencia,4,0);
                        
                        $nnum               = $this->formata_numero($this->carteira,2,0).$this->formata_numero( substr($this->nosso_numero, 0, 11)); 
                        // $nnum               = $this->formata_numero($this->carteira,2,0).$this->formata_numero($this->nosso_numero,11,0);  


                        $conta_cedente      = $this->formata_numero($this->conta,7,0); 
                        
                        /*$nnum               = $this->formata_numero($this->carteira,2,0).$this->formata_numero($nnum2,11,0);          
                        $dv_nosso_numero    = $this->digitoVerificador_nossonumero($nnum);              
                        $nnum2              = substr($nnum,0,2).'/'.substr($nnum,2).'-'.$dv_nosso_numero;


                        $dv_nosso_numero    = $this->digitoVerificador_nossonumero($nnum);
                        $conta_cedente      = $this->formata_numero($this->conta,7,0);               
                        $conta_cedente_dv   = $this->formata_numero($this->conta_dv,1,0);
    ...
    public function montar_boleto() {
        ...
        // echo "codigo de barra".$this->codigo_barras();
        // echo '<br/>';
        // echo "linha digital".$this->monta_linha_digitavel();
        // exit;
        ...
        $data = array(
            ...
            'res_endereco' => $this->res_endereco,
        );

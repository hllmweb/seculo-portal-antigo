...
class Pagamento_model extends CI_Model {
    ...
    function extrato_credito($dados){ #print_r($dados);
     ...
     function extrato_almoco($dados){
        
     $query =  $this->db->query("SELECT 
            NM_USUARIO,
            TO_CHAR(DT_MOVIMENTO,'DD/MM/YYYY') DT_MOVIMENTO,
            SALDO_ANTERIOR,
            (SALDO_ATUALIZADO - SALDO_ANTERIOR) CREDITO,
            SALDO_ATUALIZADO 
            FROM BD_CONTROLE.HIST_SALDO_ALMOCO S
            INNER JOIN BD_CONTROLE.USUARIOS U  ON U.CD_USUARIO = S.CD_USUARIO  
            WHERE S.CD_USUARIO = '".$dados['aluno']."' 
            AND TRUNC(S.DT_MOVIMENTO)  BETWEEN TO_DATE('".$dados['dt_inicio']."', 'DD/MM/YYYY') AND TO_DATE('".$dados['dt_final']."', 'DD/MM/YYYY')
            ORDER BY S.DT_MOVIMENTO");

        return $query->result();
    }
    

    function credito_almoco($dados){ 
        $cursor = ''; 
        $params = array(
            ...
            array('name' => ':P_PRECO_UNITARIO', 'value' =>  number_format($dados['vl_unitario'], 2, ',', '.')),
            array('name' => ':P_VL_TOTAL', 'value' => number_format($dados['vl_total'], 2, ',', '.')),

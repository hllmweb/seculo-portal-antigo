...
class Aula_model extends CI_Model {
    ...
    public function listaDatasPendente($periodo, $professor) {
        ...
        //$this->db->where("HR_FECHAMENTO IS NULL");
    ...
    public function listaDiasPendente($professorAtual, $disciplinas, $turmas) {
        ...
        $this->db->where("AL.LANCOU_CONTEUDO", 'N');
        $this->db->where("TRUNC(AL.DT_AULA) BETWEEN TO_DATE('28/01/2018', 'DD/MM/YYYY') AND TO_DATE('20/12/2018', 'DD/MM/YYYY')");
        $this->db->where_in("AL.CD_DISCIPLINA", $disciplinas);
        $this->db->where_in("AL.CD_TURMA", $turmas);
        //$this->db->where("AL.CONTEUDO IS NULL");
        //$this->db->where("AL.HR_FECHAMENTO IS NULL");

        //$this->db->where("PERIODO"    , '2018/1' );
        //$this->db->where(" NOT EXISTS( SELECT 1 FROM BD_SICA.FC_CALENDARIO WHERE TIPO IN ('F') AND TRUNC(DATA) = TRUNC(AL.DT_AULA)) ", '', FALSE);
    ...
    public function listaAulaDia($professor, $periodo, $subturma = null) {
        ...
        $this->db->join("BD_SICA.FC_CALENDARIO CA" , "CA.DATA = AULA.DT_AULA");

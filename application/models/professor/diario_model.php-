...
// //novo pipoco
  ...
  LEFT JOIN (SELECT  
                              ...
                              PD.CD_DISCIPLINA,
                              PD.NR_NOTA_DISCURSIVA AS TOTAL_PONTOS_DISSERTATIVAS,
                              PD.total_acertos AS TOTAL_PONTOS,
                              PD.NR_NOTA                            
                         
                          FROM BD_SICA.AVAL_PROVA_ALUNO_DISC PD
                          JOIN BD_SICA.AVAL_PROVA P ON P.CD_PROVA = PD.CD_PROVA
                          JOIN BD_SICA.AVAL_PROVA_DISC D ON P.CD_PROVA = D.CD_PROVA  AND D.TIPO_QUESTAO = 'O'
                          JOIN BD_SICA.TURMA T ON T.PERIODO = P.PERIODO AND T.CD_CURSO = P.CD_CURSO AND T.CD_SERIE = P.ORDEM_SERIE AND T.TIPO = 'N'
                          WHERE 
                          D.CD_DISCIPLINA = " . $parametro['cd_disciplina'] . "
                          AND P.NUM_NOTA = " . $parametro['numero_nota'] . "
                          AND P.PERIODO = '" . $this->session->userdata('SCL_SSS_USU_PERIODO') . "'
                          AND T.CD_TURMA = '" . $parametro['cd_turma'] . "'

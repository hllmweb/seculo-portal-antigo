...
class Tipo_Nota_model extends CI_Model {
    ...
    public function listaNotasConteudoMista($turma, $periodo) {
        ...
        $this->db->join("BD_SICA.CL_ESTRUT_NOTAS EN", "EN.CD_TIPO_NOTA = TN.CD_TIPO_NOTA AND EN.ATIVO = 1");
    ...
    /**
     * Lista todas as notas para LISTAR AS NOTAS.
     * 
     * @param string $turma
     * @param string $periodo
...
//lista as médias dos bimestres
    public function listaMediaBimestres($turma, $periodo, $disciplina, $bimestre) {
      
                
        $this->db->distinct();
        $this->db->select("AD.CD_ALUNO, AL.NM_ALUNO, AN.NOTA");
        $this->db->from("BD_SICA.CL_ALU_DISC AD");
        $this->db->join("BD_SICA.CL_ALU_NOTA AN", "AD.CD_ALU_DISC = AN.CD_ALU_DISC");
         $this->db->join("BD_SICA.ALUNO AL", "AD.CD_ALUNO = AL.CD_ALUNO");
         $this->db->where("AD.PERIODO", $periodo);
         $this->db->where("AD.CD_TURMA", $turma);
         $this->db->where("AN.NUM_NOTA", $bimestre);
         $this->db->where("CD_DISCIPLINA", $disciplina);

        ...
        <div class="col-xs-12">
            ...
            if ($navegador == 'Fiprefox') {
                ...
                <div class="panel panel-warning">                    
                    ...
                    $disc = array(29, 6, 89, 726, 22, 5, 7);
                    $sim = in_array($disciplina, $disc);

                    // if (($curso == 2) or ( $sim == true) or ( $numero_nota == 31) or ( $numero_nota == 26) && $turma != 'EMM0301') {
                  
                    //echo $curso."<br>".$sim."<br>".$numero_nota." ".$disciplina;
                    //if(($curso == 2) || (($sim == 1) && ($numero_nota == 10) && ($numero_nota == 11) || ($turma != 'EMM0101') && ($turma != 'EMM0301') && ($turma != 'EMM0201'))){
                    if(($sim == 1 && $curso == 33) || ($sim == 1 && $disciplina == 89) || ($sim == 1 && $disciplina == 5) || ($turma == 'EFM0501' && $curso == 2)){
                            ...
                            <table width="100%" class="table table-bordered table-hover" id="gridview" aria-describedby="sample-table-2_info">
                                <thead>
                                    <tr class="panel-heading">                            
                                        ...
                                        <!-- <th class="sorting_disabled text-center" style="width:12%">Objetiva</th> -->
                                        <th class="sorting_disabled text-center" style="width:12%" aria-label="">Nota Final</th>
                                    ...
                                    if (count($listar_aluno) > 0) {
                                        foreach ($listar_aluno as $row) {
                                            ...
                                            <tr>
                                                ...
                                                <td valign=" "><?= $row->NM_ALUNO ?> <?php $disabled = ($row->CD_PROVA == "") ? "disabled" : ""; ?></td>
                                                <!-- <td align="center">
                                                <?php 
                                                    // if($row->NR_NOTA != ""){
                                                    //     echo number_format($row->NR_NOTA,1,".",","); 
                                                    // }else{
                                                    //     echo "-";
                                                    // }
                                                ...
                                                <td align="center">
                                                    ...
                                                    elseif ($row->NOTA > 0) {
                                                        $valor = $row->NOTA;
                                                        //echo $valor;
                                                        echo '<input name="nota_' . $row->CD_ALU_DISC . '" placeholder="00.0" type="number" id="nota_' . $row->CD_ALU_DISC . '" place size="3" max="10" min="0.1" step=".1" onkeydown="mascara( this, mvalor );" value="'.$valor.'" />';
                                                    } elseif ($row->NOTA == '0') {
                                                        $valor = $row->NOTA;
                                                        //echo $valor;
                                                        echo '<input name="nota_' . $row->CD_ALU_DISC . '" placeholder="00.0" type="number" id="nota_' . $row->CD_ALU_DISC . '" place size="3" max="10" min="0.1" step=".1" onkeydown="mascara( this, mvalor );" value="'.$valor.'" />';
                                            ...
                                            <tr>
                                                ...
                                                <td align="center">
                                                    ...
                                                    if ($discursiva == 'error') {
                                                        ...
                                                        elseif ($discursiva > 0) {
                                                            ...
                                                            //echo '<input name="disc_'.$row->CD_ALU_DISC.'" placeholder="00.0"  type="text" id="cDisc_'.$row->CD_ALU_DISC.'" place size="5" maxlength="5" onkeyup="calculaNota('.$row->CD_ALU_DISC.');" value="'.$valor.'" onkeydown="mascara(this, mvalor);"/>';

                                                        } elseif (($discursiva == '0') or ( $discursiva != '')) {
                                                            $valor = $discursiva;
                                                            //echo number_format(str_replace(',', '.', $valor), 1, '.', '');
                                                            echo '<input name="disc_'.$row->CD_ALU_DISC.'" placeholder="00.0"  type="text" id="cDisc_'.$row->CD_ALU_DISC.'" place size="5" maxlength="5"  value="'.$valor.'" onkeydown="mascara(this, mvalor);"/>';
                                                        }elseif ($nao == false) {
                                                            $valor = $discursiva;
                                                            echo number_format(str_replace(',', '.', $valor), 1, '.', '');
                                                        } else {
                                                            ...
                                                            //efeito de nota sendo sobreposta, corrigir
                                                            ?>
                                                            <input name="disc_<?= $row->CD_ALU_DISC ?>" placeholder="00.0" type="text" step=".1" id="cDisc_<?= $row->CD_ALU_DISC ?>" place size="5" min="0.1" max="5" onkeyup="calculaNota(<?= $row->CD_ALU_DISC ?>);" onkeydown="mascara(this, mvalor);" />
                                                        <?php } ?>
                                                        <input name="CD_ALU_DISC" type="hidden" value="<?= $row->CD_ALU_DISC ?>" >   
                                                    <?php  } ?>
                                                </td>
                                                ...
                                                <td align="center">
                                                    ...
                                                    elseif ($row->NOTA > 0) {
                                                        ...
                                                        //echo '<input name="nota_' . $row->CD_ALU_DISC . '" placeholder="00.0" type="text" id="nota_' . $row->CD_ALU_DISC . '" place size="3" min="0.1" max="10" step=".1" value="'.$valor.'"/>';

                                                    } elseif ($row->NOTA == '0') {
                                                        $valor = $row->NOTA;
                                                        echo number_format(str_replace(',', '.', $valor), 1, '.', '');
                                                    } elseif ($codigoTipoNota == 34) {//Nota da MAIC
                                                        echo '<input name="nota_' . $row->CD_ALU_DISC . '" placeholder="00.0" type="text" id="nota_' . $row->CD_ALU_DISC . '" place size="3" min="0.1" max="10" step=".1"/>';
                                                    } else {
                                                        $valor = '';
                                                        echo '<input name="nota_' . $row->CD_ALU_DISC . '" placeholder="00.0"  type="text" id="nota_' . $row->CD_ALU_DISC . '" place size="5" maxlength="5"   />';
                                                    }

                                                    //echo "teste".$discursiva;
                                                    ?>

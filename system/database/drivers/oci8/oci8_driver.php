...
class CI_DB_oci8_driver extends CI_DB {
    ...
    function sp_seculo($package, $procedure, $params) {
        ...
        foreach ($params as $param) {
            ...
            if ($param['type'] == OCI_B_CURSOR) {
                ...
                oci_bind_by_name($stmt, $param['name'], $retorno, 20);
            } else {
        ...
        if ($query == FALSE) {
            ...
            if (count($cursor) > 0) {
                ...
                return(array('retorno' => $retorno, 'cursor' => $cursor));
            }
        }
    }

    function queryblob($sql,$params,$pos) {
        
       // print_r($_FILES["img"]["tmp_name"][$pos]);exit;
        ...
        foreach ($params as $param) {
            if (isset($param['type'])) {
                oci_bind_by_name($stmt, $param['name'], $lob, -1, OCI_B_BLOB);
            }
        }
        $query = ociexecute($stmt, OCI_DEFAULT);
        if($lob->save($imagem)){
    ...
    function procedure($package, $procedure, $params) {
        ...
        foreach ($params as $param) {
            if ($param['type'] != OCI_B_CURSOR) {
                ...
                oci_bind_by_name($stmt, $param['name'], $param['value']);
            } elseif ($param['type'] == OCI_B_CURSOR) {
    ...
    function fctseculo($package, $procedure, $params, $declare, $length) {
        if ($package == '' OR $procedure == '' OR !is_array($params)) {
            if ($this->db_debug) {
                log_message('error', 'Invalid query: ' . $package . '.' . $procedure);
                return $this->display_error('db_invalid_query');
        ...
        // build the query string
        $sql = "begin $declare := $package.$procedure(";
        ...
        // VERIFICA A VARIÁVEL DE RETORNO
        oci_bind_by_name($stmt, $declare, $length);
        // VERIFICA OS CAMPOS DA SP
        foreach ($params as $param) {
            oci_bind_by_name($stmt, $param['name'], $param['value']);
        }
        ...
        if ($query == FALSE) {
            ...
            $retorno = oci_free_statement($stid);
            oci_commit($this->conn_id);
            return $retorno;
        }
    }
